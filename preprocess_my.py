...
def process_file(file_path, data_file_role, dataset_name, word_to_count, path_to_count, max_contexts):
    ...
    empty = 0
    max_unfiltered = 0
    output_path = '{}.filtered'.format(file_path)
...
if __name__ == '__main__':
    ...
    # for data_file_path, data_role in zip([test_data_path, val_data_path, train_data_path], ['test', 'val', 'feature']):
    for file_path in sys.stdin:
        file_path = file_path.strip()
        data_role = 'none'
        num_examples = process_file(file_path=file_path, data_file_role=data_role, dataset_name=args.output_name,
